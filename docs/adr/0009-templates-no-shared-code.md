...
4. **Each template directory is self-contained, imports only `@nulfrog/sandcastle`** (chosen). One-line rule, easy to apply on review, keeps `init` a pure directory copy, and lets each template evolve independently.
